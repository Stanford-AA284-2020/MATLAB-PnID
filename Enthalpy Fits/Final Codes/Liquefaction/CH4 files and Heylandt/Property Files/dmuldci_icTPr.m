function dmuldci = dmuldci_icTPr(i,c,T,P,rl)
...
if(c(i) < cinc)                             % Use a forward difference.
    clow = c;
    rl = rl_cTP(clow,T,P,rl,1);
    glow = mui_icrT(i,clow,rl,T);           % Chemical potential function.
    ...
    rl = rl_cTP(cmid,T,P,rl,1);
    gmid = mui_icrT(i,cmid,rl,T);           % Chemical potential function.  
    ...
    rl = rl_cTP(chigh,T,P,rl,1);
    ghigh = mui_icrT(i,chigh,rl,T);         % Chemical potential function.

    % Use a second-order finite-difference formula.  See Ferziger's book on
    % numerical methods for details.
    
    dmuldci = (4*gmid - ghigh - 3*glow)/(2*cinc);
    return
end
