...
[Tnumcrit rnumcrit] = Critical_i(CH4)
Pnumcrit = P_irT(CH4,rnumcrit,Tnumcrit)
Tnumtrip = Ttrip_i(CH4);
[Pnumtrip rfnumtrip rgnumtrip] = Saturation_iT(CH4,Tnumtrip)
fprintf('\n')
fprintf('               T trip(K)    P trip(Pa)   rf trip(kg/m3)  rg trip(kg/m3)\n')
fprintf('-----------------------------------------------------------------------\n')
fprintf('Experimental     %5.2f        %6.2f        %6.4f        %6.6f\n',...
    Ttrip_i(CH4),Ptrip_i(CH4),rftrip_i(CH4),rgtrip_i(CH4))
...
fprintf('Experimental    %5.4f       %5.0f      %5.5f\n',...
    Tcrit_i(CH4),Pcrit_i(CH4),rcrit_i(CH4))
...
for T=Tmin:dT:Tmax
    ...
    rfs = Liquid_Spinodal_iT(CH4,T);
    Pfs = P_irT(CH4,rfs,T);
    rgs = Vapor_Spinodal_iT(CH4,T);
    Pgs = P_irT(CH4,rgs,T);
...
Tlist = [...
    Ttrip_i(CH4)...
    0.1*(Tcrit_i(CH4)-Ttrip_i(CH4)) + Ttrip_i(CH4)...
    0.25*(Tcrit_i(CH4)-Ttrip_i(CH4)) + Ttrip_i(CH4)...
    0.5*(Tcrit_i(CH4)-Ttrip_i(CH4)) + Ttrip_i(CH4)...
    0.75*(Tcrit_i(CH4)-Ttrip_i(CH4)) + Ttrip_i(CH4)...
    0.9*(Tcrit_i(CH4)-Ttrip_i(CH4)) + Ttrip_i(CH4)...
    0.95*(Tcrit_i(CH4)-Ttrip_i(CH4)) + Ttrip_i(CH4)...
    1.00*(Tcrit_i(CH4)-Ttrip_i(CH4)) + Ttrip_i(CH4)...
    1.05*(Tcrit_i(CH4)-Ttrip_i(CH4)) + Ttrip_i(CH4)...
    1.25*(Tcrit_i(CH4)-Ttrip_i(CH4)) + Ttrip_i(CH4)...
    1.50*(Tcrit_i(CH4)-Ttrip_i(CH4)) + Ttrip_i(CH4)...
    2.0*(Tcrit_i(CH4)-Ttrip_i(CH4)) + Ttrip_i(CH4)...
    0.1*(Tupper_i(CH4)-Tcrit_i(CH4)) + Tcrit_i(CH4) ...
    0.4*(Tupper_i(CH4)-Tcrit_i(CH4)) + Tcrit_i(CH4) ...
    Tupper_i(CH4)...
    ];
...
for j=1:1:length(Tlist)
    ...
    if(T >= Tnumcrit)
        ...
        i = 1;
        % Liquid side...
        lvmax = log(1/rcrit_i(CH4));
        lvmin = log(1/rftrip_i(CH4));
        ...
        for lv=lvmin:ldv:lvmax
            v = exp(lv);
            Pvlisotherm(j,i) = P_irT(CH4,1/v,T);
            vlisotherm(j,i) = v;
            i = i+1;
        end
        % Vapor side...
        lvmax = log(1/rgtrip_i(CH4));
        lvmin = log(1/rcrit_i(CH4));
        ldv = (lvmax-lvmin)/steps/2;
        % Nudge the bound out a bit.
        lvmax = lvmax + ldv;
        ldv = (lvmax-lvmin)/steps/2;
        for lv=lvmin:ldv:lvmax
            v = exp(lv);
            Pvgisotherm(j,i) = P_irT(CH4,1/v,T);
            vgisotherm(j,i) = v;
            i = i+1;
        end
    else
        ...
        i = 1;
        % Liquid side...
        lvmax = log(1/Liquid_Spinodal_iT(CH4,T));
        lvmin = log(1/rftrip_i(CH4));
        ldv = (lvmax-lvmin)/steps/2;
        % Nudge the bound out a bit.
        lvmin = lvmin - ldv;
        ...
        % Vapor side...
        lvmax = log(1/rgtrip_i(CH4));
        lvmin = log(1/Vapor_Spinodal_iT(CH4,T));
        ldv = (lvmax-lvmin)/steps/2;
        % Nudge the bound out a bit.
        lvmax = lvmax + ldv;
        ldv = (lvmax-lvmin)/steps/2;
...
figure(1)
clf
semilogx(1/rcrit_i(CH4),Pcrit_i(CH4)/1e6,'bd')
hold on
...
for i=2:1:length(Tlist)-1
    text(2,1.4*(i)*(Pcrit_i(CH4)/1e6)/(length(Tlist)),num2str(Tlist(i),'%.2f'))
end
i = 1;
text(2,1.4*(i)*(Pcrit_i(CH4)/1e6)/(length(Tlist)),num2str(Tlist(i),'%.2f'),'Color','r')
i = length(Tlist);
text(2,1.4*(i)*(Pcrit_i(CH4)/1e6)/(length(Tlist)),...
    ['T = ',num2str(Tlist(i),'%.0f'),' K'],'Color','r')
vmax = 1/rgtrip_i(CH4);
vmin = 1/rftrip_i(CH4);
scale = axis;
axis([scale(1) scale(2) -.25 10])
plotfixer
...
for j=1:1:length(Tlist)
    ...
    if(T >= Tcrit_i(CH4))
        ...
        dr = (rmax-rmin)/steps/2;
        for r=rmin:dr:rmax
            Plisotherm(j,i)  = P_irT(CH4,r,T);
            mulisotherm(j,i) = mu_irT(CH4,r,T);
            i = i+1;
        end
        % Vapor side...
        rmax = rcrit_i(CH4);
        ...
        dr = (rmax-rmin)/steps/2;
        for r=rmin:dr:rmax
            Pgisotherm(j,i)  = P_irT(CH4,r,T);
            mugisotherm(j,i) = mu_irT(CH4,r,T);
            i = i+1;
        end
    else
        % Split into two parts--limit at the spinodals.
...
mucrit = mu_irT(CH4,rcrit_i(CH4),Tcrit_i(CH4))
mutrip = mu_irT(CH4,rgtrip_i(CH4),Ttrip_i(CH4))
munumcrit = mu_irT(CH4,rnumcrit,Tnumcrit)
munumtrip = mu_irT(CH4,rgnumtrip,Ttrip_i(CH4))

figure(2)
clf
hold on
plot(Pcrit_i(CH4)/1e6,mucrit/1e6,'rd')
...
for i=2:1:length(Tlist)-1
    text(1.4,.5*(2-i)*(Pcrit_i(CH4)/1e6)/length(Tlist),num2str(Tlist(i),'%.2f'))
end
i = 1;
text(1.4,.5*(2-i)*(Pcrit_i(CH4)/1e6)/length(Tlist),...
