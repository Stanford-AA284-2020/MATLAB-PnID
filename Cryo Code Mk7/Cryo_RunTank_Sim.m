...
dt = 0.001; % seconds
V0 = 0.6; % cubic feet (Internal volume)
V0 = V0*0.0283168; % Convert to cubic meters 

% Compute the initial mass, volume and moles of nitrogen and oxygen
r_N2_0 = rv_iTP(N2, T0, P0); % kg/m^2: Nitrogen is expected to be supercritical
r_O2_0 = rl_iTP(O2, T0, P0); %  kg/m^3: Oxygen is a subcooled liquid
...
time = 0:dt:5; P = zeros(1, length(time)); 
T =  zeros(1, length(time)); N_O2_L = zeros(1, length(time));
N_O2_V = zeros(1, length(time)); mdot = zeros(1, length(time));

T(1,1) = T0; P(1,1) = P0; N_O2_L(1,1) = N_O2_0; N_O2_V(1,1) = 0;
m_O2_L = 0.032*N_O2_L; m_O2_V = 0.032*N_O2_V; mdot(1,1) = mdot_0;

for t = dt:dt:1
    
    i = floor(t/dt) + 1;
    if t > 0
        P_sat = Saturation_iT(O2, T(1, i-1));
        dm = mdot(1, i-1)*dt;
        m_O2_L(1, i) = m_O2_L(1, i-1) - dm;
        r_O2_L = rl_iTP(O2, T(1, i-1), P(1, i-1));
        dq = dm*h_irT(O2, r_O2_L, T(1, i-1)); % Enthalpy carried by the discharged liquid
    
    
    if abs((P(1, i-1) - P_sat)/P_sat) < toler
        % If oxygen is in the liquid-vapor coexistence regime
        disp('Inside the vapordome');
        v_O2_L = 1/rl_iTP(O2, T(1, i-1), P(1, i-1));
        ...
        for j=1:1:NFPs
            ...
            % vapor phase
            x_O2_V = (P_sat/Pnext)*exp((v_O2_L/(8.314*T(1,i-1)/0.032))*(Pnext - P_sat));
            x_N2_V = 1 - x_O2_V; 
        ...
        P(1, i) = P_solution; T(1, i) = T_solution;
        ...
        dV = dm/r_O2_L;
        v_N2_now = (m_N2_0/rv_iTP(N2, T(1, i-1), P(1, i-1))) + dV; % Imposing the volume constraint
        Thigh = T(1, i-1)*1.2; Tlow = T(1,i-1)*0.8;
        s_N2_high = s_irT(N2, m_N2_0/v_N2_now, Thigh); 
        s_N2_low = s_irT(N2, m_N2_0/v_N2_now, Tlow); 
        s_N2_previous = s_irT(N2, m_N2_0/(V_N2_0), T(1, i-1)); % Entropy at the previous time step
...
%         end
        ...
        P(1, i) = P_solution; T(1, i) = T_solution;
        m_O2_L(1,i) = m_O2_L(1, i-1) - dm; m_O2_V(1,i) = 0; mdot(1,i) = mdot_0;
        N_O2_L(1,i) = m_O2_L(1,i)/0.032; N_O2_V(1,i) = 0;
