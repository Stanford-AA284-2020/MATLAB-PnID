function rgs = Vapor_Spinodal_iT(ispecies,T,varargin)
% Return the density of the vapor-side spinodal line rgs (kg/m3) for
% species i at temperature T (K).
% If the temperature is above critical or the spinodal cannot be found,
% return a value of zero for the density and complain.
% C.F. Edwards, 2-3-10

global toler
global Tcrit_i rcrit_i rgtrip_i
...
% Check for T > Tcrit.
if(T > Tcrit)
%     disp('Temperature above critical in Vapor_Spinodal_iT')
    rgs = 0;
    return
end
...
if(i == imax)
    % Nope.  Sorry!
%     disp('Spinodal not found in Vapor_Spinodal_iT')
    rgs = 0;
    return
else
    % Yep!  Here it is.
...
if(j == jmax)
%     disp('Backtrack not successfull in Vapor_Spinodal_iT')
end
