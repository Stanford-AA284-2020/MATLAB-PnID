% Make P-T and lnP vs. -1/T diagrams for N2.  
% C.F. Edwards, 2/6/10 
...
gcrit_num = g_irT(N2,rcrit_num,Tcrit_num)

% Get the numerical triple point data.
Ptrip_exp = Ptrip_i(N2)
rftrip_exp = rftrip_i(N2)
rgtrip_exp = rgtrip_i(N2)
% The numerical triple line uses the experimental triple-point temperature
% and must satisfy saturation conditions.
[Ptrip_num rftrip_num rgtrip_num] = Saturation_iT(N2,Ttrip_i(N2))
gftrip_num = g_irT(N2,rftrip_num,Ttrip_i(N2))
ggtrip_num = g_irT(N2,rgtrip_num,Ttrip_i(N2))
format short
...
figure(1)
clf
plot([Ttrip_i(N2) Tcrit_i(N2)],...
    [Ptrip_i(N2)/1e6 Pcrit_i(N2)/1e6],'bo')
hold on
plot([Ttrip_i(N2) Tcrit_num],...
...
figure(2)
clf
semilogy(-[1/Ttrip_i(N2) 1/Tcrit_i(N2)],...
    [Ptrip_i(N2)/1e6 Pcrit_i(N2)/1e6],'bo')
hold on
semilogy(-[1/Ttrip_i(N2) 1/Tcrit_num],...
    [Ptrip_num/1e6 Pcrit_num/1e6],'bx')
